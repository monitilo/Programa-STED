...
from pyqtgraph.console import ConsoleWidget  # Ger- no la usaba y la saque
        ...
        params = [{'name': 'Model', 'type': 'str',
                  ...
                  {'name': 'Acquisition mode', 'type': 'group', 'children': [
                      {'name': 'Trigger source', 'type': 'list',
                       'values': ['External "frame-trigger"',  # los cambie
                                  'Internal trigger',           # de lugar
...
#        self.resize(2400, 1300)
        ...
        self.recWidget = RecordingWidget(self)  # LuchoScanWidget(self)  # Ger -----------------------------------------------------aca---
...
# ---Agregado por german-----------------------------
        ...
        # Confocal
        confocDock = Dock('Confocando!!', size=(1, 1))
        self.confocWidget = scanner.ConfocalWidget(self.nidaq, self)
        confocDock.addWidget(self.confocWidget)
        dockArea.addDock(confocDock, 'below', FocusLockDock)
...
# ---Agregado por german-----------------------------
        ...
        layout.addWidget(cameraWidget, 1, 0, 3, 2)  # 1,0,2,2)
        layout.addWidget(self.viewCtrl, 4, 0, 1, 2)  # 3,0,1,2)
        layout.addWidget(self.recWidget, 5, 0, 1, 2)  # 4,0,1,2)
#        layout.addWidget(console, 5, 0, 1, 2)  # GEr- me molestaba a la vista
        layout.addWidget(imageWidget, 0, 2, 6, 1)
        layout.addWidget(illumDockArea, 0, 3, 2, 1)
        layout.addWidget(dockArea, 2, 3, 4, 1)

        layout.setRowMinimumHeight(2, 175)  # (2, 175) Ger- toy cmbiando tamaño
        layout.setRowMinimumHeight(3, 100)  # (3, 100)
        layout.setRowMinimumHeight(5, 175)  # (5, 175)
        layout.setColumnMinimumWidth(0, 275)  # (0, 275)
        imageWidget.ci.layout.setColumnFixedWidth(1, 550)  # (1, 1150)
        imageWidget.ci.layout.setRowFixedHeight(1, 750)  # (1, 1150)
        layout.setColumnMinimumWidth(2, 700)  # (1, 1350)
        # Ger-  3 lineas mas para acomodarme la pantalla
        self.resize(20, 20)
        self.move(50, 80)
        self.showMaximized()
...
#        self.initialPositionEdit = QtGui.QLineEdit('10 10')
...
#        self.numberofPixelsEdit = QtGui.QLineEdit('128')
