...
#            print("listo el pollo")
    ...
    def updateView(self):
        ...
        self.image[:, self.numberofPixels-2-(self.dy)] = np.flip(lineData2, axis=0)
...
#        self.aotask.write(
#           self.fullsignal, auto_start=True)
...
#        APD = np.split(np.array(APDtodo), 2*self.numberofPixels)
        ...
        for j in range(1, 2*self.numberofPixels-1):
...
#                    aux = aux + 1
#            self.cuentas[j] = aux + np.random.rand(1)[0]*0


        print("\n", "ditask posta", np.round(toc-tac, 5)/(2*self.numberofPixels), "pixeltime = ", self.pixelTime)
        print("data posta", np.round(time.time() - tic, 5)/2, "linetime = ", self.linetime)
...
#          self.fullsignal = np.zeros((len(self.AOchans),len(self.barridox)))
#
#          self.fullsignal[0,:]= self.barridox
#          self.fullsignal[1,:]= self.barridoy
#          self.fullsignal[2,:]= self.barridoz

#          self.done()
#          self.channelsOpen()

#    def updatebarrido(self):
#        self.done
#        self.channelsOpen()
#        self.fullsignal[1,:]= self.barridoy + (self.dy * self.pixelSize / convFactors['y'])
...
#                lines="Dev1/port0/line7",  # name_to_assign_to_lines='chan2.0',
...
#            self.aotask.in_stream.input_buf_size = 2
#            self.aotask.out_stream.output_buf_size = 10**8  # self.numberofPixels*2
