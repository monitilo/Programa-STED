...
#            self.scanParameterChanged('scanMode')
        ...
        self.triggerEdit = QtGui.QLineEdit('500')
...
#        Nramp = Ntirar + Npix  # len(barridox)  # len(rampay)  # el largo de cada rampa sola (completa).
...
#            self.ditask = nidaqmx.Task('ditask')  # Ger
    ...
    #            line_grouping=nidaqmx.constants.LineGrouping.CHAN_PER_LINE)  # Ger
            ...
            trigger = np.zeros((2*num),dtype="bool")  # np.zeros((10), dtype="bool")  # np.zeros((len(signaltodo)),dtype="bool")

            trigger[num:] = True

            self.trigger = trigger
            print("\n", len(trigger), "len trigger", num, "num elegido")
