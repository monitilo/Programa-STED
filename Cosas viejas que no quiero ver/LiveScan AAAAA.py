...
#            print("listo el pollo")
    ...
    def updateView(self):
        ...
        self.image = np.zeros((len(self.barridonormal), self.numberofPixels))
#        self.imagevuelta = self.image
...
#            for i in range(self.numberofPixels):
#                ef = ((i+1) ) + self.pixelsoffL
#                ei = ((i+0) ) + self.pixelsoffL
...
#        self.imagevuelta[:, self.numberofPixels-1-(self.dy)] = np.flip(lineDatavuelta, axis=0)
    ...
    def preRunTime(self):
        self.sampleRate = self.apdrate  # (self.scanRange / resolucionDAQ) / (self.linetime)
...
#        self.aotask.triggers.start_trigger.dig_edge_edge = nidaqmx.constants.Edge.RISING
#        self.aotask.triggers.start_trigger.delay_units = nidaqmx.constants.DigitalWidthUnits.TICKS
#        self.aotask.triggers.start_trigger.delay = 310000
...
#                source=r'100kHzTimeBase',
        ...
        self.APDtodo = self.citask.read(len(self.barridox))
        self.aotask.start()
        tiic = time.time()
#        for i in range(self.numberofPixels):
#            tac = time.time()
...
##            self.citask.wait_until_done()
#            self.contando[:,i] = self.citask.read(int(len(self.barridoxchico)))  # APDtodo
#            self.citask.wait_until_done()
#            toc = time.time()
...
#barridox =barridoxchico*Npix
#APDtodo = barridox

        contando = np.zeros((len(self.barridonormal)*Npix))
        s=0
        l=0
        for j in range(Npix):
        #    l = (j*Npix)-1
#            l=s
            for i in range((len(self.barridonormal)-1)):
            ...
            s = s + len(self.barridonormal)
            l= l +len(self.barridoxchico)
