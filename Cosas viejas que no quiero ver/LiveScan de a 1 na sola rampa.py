...
#            print("listo el pollo")
    ...
    def updateView(self):
        ...
        imagen = np.split(self.contando, self.numberofPixels)
...
#        """
    ...
    #
            tac = time.time()
            self.APDtodo = self.citask.read(int(self.Napd* self.numberofPixels * (self.numberofPixels +init)))
    ...
    #
            ...
            print("completo real (s) = ", np.round((tuc - tic), 4))

            print("\n timeReal", self.timeReal,"\nlinetime=", self.linetime)
            print("\ncitask solo Real (ms)= ", np.round((toc-tac)*10**3, 4))
...
#
        self.aotask.wait_until_done()
#
#        self.cuentas[0] = self.APDtodo[(self.Napd - 1)+(self.Napd*init)]
#        for j in range(1, self.numberofPixels*self.numberofPixels):
#            self.cuentas[j] = self.APDtodo[(((j+1)*self.Napd)-1)+(self.Napd*init)] - self.APDtodo[(((j)*self.Napd)-1)+(self.Napd*init)]
...
#        print(self.dy, "dy")
        ...
        self.citask.stop()
        tuc = time.time()
        print("\ncitask solo Realtotal(s) = ", np.round((toc-tac), 4),
              "T total imagen supuesto (s)= ", self.linetime*self.numberofPixels)
        print("imagen Real (s)= ", np.round((tuc - tic), 4),
...
#        self.barridoy = np.ones(len(self.barridox))*startY / convFactors['y']
        ...
        p = len(self.xini) + len(barridonormal) + len(self.xfin[1:]) + int(len(self.xvuelta[1:-1])/2)
        for i in range(self.numberofPixels):
...
#        Ntotal = Nramp * Npix  # len(todo)  # len(barridoy)  # que son todos los puntos de las rampas, despues corto.
        ...
        contando = np.zeros((Npix*Npix))
        s=0
        for j in range(Npix-1):
            l = ((j*(Nramp*Napd)))  # avanza de a rampas
            for i in range(Npix-1):
                ef = ((i+2)*Napd)-1  + NoffL  # avanza de a pixels
                ei = ((i+1)*Napd)-1  + NoffL  # avanza de a pixels
                contando[(i)+s] = self.APDtodo[(ef+l)]- self.APDtodo[(ei+l)]
        #        print(contando[(i)+s])
            s=s+Npix
        #    print("-s-        ",s)
        #print(contando, "<@>")
...
#        self.imagin = np.split(contando, Npix)
    ...
    def channelsOpen(self):
        if self.canales:
            ...
            self.citask.timing.cfg_samp_clk_timing(
              ...
              samps_per_chan = int(self.Napd *(self.numberofPixels+self.pixelsoff) * self.numberofPixels)) # + self.pixelsoffini)))
