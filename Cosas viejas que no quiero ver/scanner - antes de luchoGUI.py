...
#        # cambie dotask para que simule ser un APD, cuando ande uso uno posta
...
# --- StepScan---
    ...
    def StepScan(self):  # si step
        ...
        tapd = 10**-5
        apdrate = 9*10**6  # self.main.sampleRate
...
#        self.sampleRate = 10**5
        ...
        self.cuentas = np.zeros((self.seqSamps, stepsY))
        ...
        for j in range(stepsY):
            ...
            for i in range(self.seqSamps):
                ...
                for c in range(Napd-1):
                    if APD[c] < APD[c+1]:
                        aux = aux + 1
                self.cuentas[i, j] = aux + i+j  # solo para probar el grafico.
                self.plotUpdate()  # dibuja en vivo
            toc = time.time()
...
# --- dinamic plot------------
    ...
    def plotPrepare(self):
        ...
        self.X, self.Y = np.meshgrid(self.barridox, self.barridoy)

#        fig, ax = plt.subplots()
        self.axes.pcolor(self.X, self.Y, self.cuentas, cmap=plt.cm.jet)  # , vmin=0)
#        cb = fig.colorbar(p)
...
#        plt.show()
    ...
    def plotStop(self):
        if self.ploting:
            ...
            print('Cannot stop when not running (from dibujo nuevo :)')
...
def smoothRamp(start, end, samples):
    ...
    return signal
# --- Barrido dinamico que no me sale ---
...
#
#    def compute_initial_figure(self):
##        self.axes.plot([0, 1, 2, 3], [1, 2, 0, 4], 'g')
#
#        self.L = L = 20
#        self.N = N = 20
#        self.a = np.zeros((L, L))
##        self.a[8, 8] = 10 
##        self.signal = np.zeros((L, L, N), dtype='bool')
#        x = np.linspace(0, 1, L)
#        y = x
#        X, Y = np.meshgrid(x, y)
...
#
##        cb = fig.colorbar(p)
#        self.axes.pcolor(X, Y, self.a, cmap=plt.cm.jet, vmin=0)
##        p = self.axes.pcolor(X, Y, self.a, cmap=plt.cm.jet, vmin=0)
#        self.axes.set_xlabel('x [um]')
#        self.axes.set_ylabel('y [um]')
...
##        r = [random.randint(0, 10) for i in range(1, 2)]
#
#        L = self.L
#        N = self.N
##        a = np.zeros((L, L))
...
#        a = self.a
#        if i == N-2:
#            p = self.axes.pcolor(X, Y, a, cmap=plt.cm.jet)  #, vmin=0, vmax=2)
#            self.figure.colorbar(p)
##            cb = fig.colorbar(p)
#            self.i = i+1
#
#        else:
#            self.signal = np.random.rand(N, L, L)
##            for o in range(5):
##                for j in range(5):
##                    for e in range(N):
##                        self.signal[o+1, j+1, e] = True
#
#    #        for i in range(L):
#            for j in range(L):
#                for p in range(N-1):
#                    if self.signal[i, j, p] < self.signal[i, j, p+1]:
#                        a[i, j] = a[i, j] + 1
#            self.a = a
#            self.i = i+1
#            self.axes.set_xlabel('x [um]')
#            self.axes.set_ylabel('y [um]')
#            self.axes.pcolor(X, Y, a, cmap=plt.cm.jet)
#
#
#    #        p = self.axes.pcolor(X, Y, a, cmap=plt.cm.jet, vmin=0)
#            self.draw()