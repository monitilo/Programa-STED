...
#            self.step = True
        plt.plot(self.barridox)
#        plt.plot(self.barridoychico)
        plt.show()
...
#        self.i = 0
        self.dy = 0
        self.image = np.zeros((len(self.barridox), self.numberofPixels))
...
#                self.dy = 0
    ...
    def preRunTime(self):
        ...
        tac = time.time()
        APDtodo = self.citask.read(int(self.Napd *self.numberofPixels* (self.numberofPixels + init)))
...
#        print(len(APDtodo), "APDtodo")
        ...
        self.cuentas[0] = APDtodo[(self.Napd - 1)+(self.Napd*init)]
        for j in range(1, self.numberofPixels):
            self.cuentas[j] = APDtodo[(((j+1)*self.Napd)-1)+(self.Napd*init)] - APDtodo[(((j)*self.Napd)-1)+(self.Napd*init)]

        tuc = time.time()
        self.timeReal = (toc - tac) / self.numberofPixels
        print("\ncitask solo Real = ", np.round((toc-tac)*10**3, 2),
              "pixeltime (ms)= ", self.pixelTime*10**3)
        print("linetime Real = ", np.round((tuc - tic)*10**3, 2),
...
#        """
    ...
    def lineaida(self):
        ...
        trigger = np.zeros((len(self.barridox)),dtype="bool")  # np.zeros((10), dtype="bool")  # np.zeros((len(signaltodo)),dtype="bool")
        trigger[-2] = True
...
#                                trigger_edge = nidaqmx.constants.Edge.RISING)
        ...
        self.citask.triggers.arm_start_trigger.dig_edge_src = "PFI0"
...
#        
        self.aotask.write(
           np.array([self.barridox,
            self.barridoy + (self.dy * self.pixelSize / convFactors['y']),
            self.barridoz]), auto_start=False)
        ...
        self.dotask.write(trigger,auto_start=True)

        APDtodo = self.citask.read(len(self.barridox))
...
#        self.aotask.wait_until_done()
        ...
        self.cuentas = np.zeros((len(APDtodo)))

        self.cuentas[0] = APDtodo[(self.Napd - 1)+(self.Napd*init)]
        for j in range(1, len(APDtodo)-2):
            self.cuentas[j] = APDtodo[(((j+1)*self.Napd)-1)+(self.Napd*init)] - APDtodo[(((j)*self.Napd)-1)+(self.Napd*init)]
...
#        print(self.dy, "dy")
        ...
        self.citask.stop()
        tuc = time.time()
#        print("\ncitask solo Real = ", np.round((toc-tac), 5)*10**3,
#              "T total imagen (ms)= ", self.linetime*10**3)
#        print("linetime Real (ms)= ", np.round((tuc - tic), 5)*10**3,
...
#        self.cuentasvuelta = np.zeros((self.numberofPixels))
        ...
        self.barridox = np.concatenate((self.xini,
                                             barridonormal,
                                             self.xfin[1:],
                                             self.xvuelta[1:-1],
...
#        tiempofin = abs(tcasi/(xcasi / resolucionDAQ))
        ...
        print(self.pixelsoffini, "pixelsoffini")
    ...
    def channelsOpen(self):
        if self.canales:
            ...
            self.citask.timing.cfg_samp_clk_timing(
              rate=self.apdrate, sample_mode=nidaqmx.constants.AcquisitionType.FINITE,
              source='100kHzTimebase',
              samps_per_chan = int(self.Napd * self.numberofPixels)) # + self.pixelsoffini)))
