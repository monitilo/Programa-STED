...
# This is the function triggered by pressing the liveview button
    ...
    def liveviewStart(self):
        if self.scanMode.currentText() == "step scan":
            ...
            self.iniciando()
            self.tic = time.time()
            self.viewtimer.start(self.otrotime*10**3)
        else:
...
#        self.dotask.write(self.trigger)
...
#        self.dotask.write(False)

#        print((time.time()-tac)*10**3, "todo citask (ms)\n")  # , tec-tac,"sin el wait\n",
#              time.time()-tec, "solo el wait\n")

#        print("APD en i+1", self.contando[-1,self.dy])
        ...
        if self.batata in multi5:
            self.img.setImage(self.image, autoLevels=True)

#fifo graficos

#        print(time.time()-toc, "time")
...
#            self.ditask = nidaqmx.Task('ditask')  # Ger
    ...
    #            line_grouping=nidaqmx.constants.LineGrouping.CHAN_PER_LINE)  # Ger
            ...
            trigger = np.zeros((len(self.barridoxchico)),dtype="bool")  # np.zeros((10), dtype="bool")  # np.zeros((len(signaltodo)),dtype="bool")

            trigger[:] = True
#            trigger1 = np.concatenate((trigger, np.zeros(100,dtype="bool")))  # 2ms de apagado
