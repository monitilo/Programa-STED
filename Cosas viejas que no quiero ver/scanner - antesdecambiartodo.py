...
class ConfocalWidget(QtGui.QMainWindow):
    ...
    def __init__(self, device, main, *args, **kwargs):
        ...
        self.scanRadio = QtGui.QRadioButton('SScan')
        ...
        self.scanButton = QtGui.QPushButton('Confocan')
        self.scanning = False
        self.scanButton.clicked.connect(self.scanOrAbort)
        self.otroscanButton = QtGui.QPushButton('Otroscan')
        self.scanning = False
        self.otroscanButton.clicked.connect(self.scanOrAbort2)
        ...
        grid.addWidget(self.otroscanButton, 0, 7, 1, 2)
    ...
    def prepAndRun(self, continuous=False):
        ...
        if self.scanRadio.isChecked():
            ...
            self.scanButton.setText('Run')
            del self.lasercycle
# --- tuve que cambiar muy sutilmente algo, y para eso copie y pegue otro

    def scanOrAbort2(self):
        if not self.scanning:
            main = self.main
            lasers = main.laserWidgets
            if (lasers.DigCtrl.DigitalControlButton.isChecked() and
    ...
    def prepAndRun2(self, continuous=False):
        ''' Only called if scanner is not running (See scanOrAbort function).
        '''
        if self.scanRadio.isChecked():
            self.stageScan.update(self.scanParValues)
            self.otroscanButton.setText('Waito')
            if not(continuous):
                self.main.piezoWidget.resetChannels(
                    self.stageScan.activeChannels[self.stageScan.scanMode])
            self.scanner = Otroscanner(
               self.nidaq, self.stageScan, self.pxCycle, self, continuous)
            self.scanner.finalizeDone.connect(self.finalizeDone2)
            self.scanner.scanDone.connect(self.scanDone2)
            self.scanning = True
            if self.focusLocked:
                self.focusWgt.unlockFocus()
            ...
            self.scanner.runScan2()

        elif self.otroscanButton.isChecked():
            self.lasercycle = LaserCycle(self.nidaq, self.pxCycle)
            self.otroscanButton.setText('Stopp')
            self.lasercycle.run()

        else:
            self.lasercycle.stop()
            self.otroscanButton.setText('Runn')
            del self.lasercycle

    def scanDone2(self):
        self.otroscanButton.setEnabled(False)
    ...
    def finalizeDone2(self):
        if (not self.continuousCheck.isChecked()) or self.scanner.aborted:
            self.otroscanButton.setText('VOLVEEE')
            self.otroscanButton.setEnabled(True)
            del self.scanner
            self.scanning = False
            if self.focusLocked:
            ...
            self.otroscanButton.setEnabled(True)
            self.prepAndRun2(True)
        else:
            self.otroscanButton.setEnabled(True)
            self.prepAndRun2()

    def updateScan2(self, devices):
        self.stageScan.update(self.scanParValues)
        self.pxCycle.update(devices, self.pxParValues, self.stageScan.seqSamps)

    def closeEvent2(self, *args, **kwargs):
        try:
...
class Confocanner(QtCore.QObject):
    ...
    def __init__(self, device, stageScan, pxCycle, main, continuous=False,
        ...
        self.aitask.ai_channels.add_ai_voltage_chan(
            ...
            name_to_assign_to_channel='chan_in',
...
#        """cambie dotask para que simule ser un APD, cuando ande algo uso un
#        APD posta"""
...
#        self.ConfocalWidget.updateScan(self.allDevices)
        ...
        ax0.plot(self.stageScan.sigDict['x'], '.-', label="x")  # * convFactors['x'], label="x")
        ax0.plot(self.stageScan.sigDict['y'], '.-', label="y")  # * convFactors['y'], label="y")
...
#        plt.axis([0, 40, -0.1, 1.1])
        print(self.pxCycle.sampleRate, "sampleRate", self.sampsInScan, "sampsInScan")
...
class LineScan():
    ...
    def update(self, parValues, primScanDim):
        ...
        self.sigDict[primScanDim] = 1.14 * ramp  # Ger- ¿Por que 1.14?
...
class FOVscan():
    ...
    def update(self, parValues, primScanDim):
        ...
        at the end.'''
        # Create signals
        startX = startY = 0 # self.Positionner.x  # a confirmar si es asi. NO
#        startY = self.Positionner.y
        sizeX = parValues['sizeX']
        sizeY = parValues['sizeY']
        stepSizeX = parValues['stepSizeXY']
        stepSizeY = parValues['stepSizeXY']
    ...
    def makePrimDimSig(self, dim, start, size, steps, otherSteps):
        ...
        LTRramp = makeRamp(start, size, rowSamps)  # left to Rigth (creo) <--
        # Fast return to startX
        RTLramp = makeRamp(size, start, self.seqSamps)  # Rigth to left (creo)
...
# ---Agregando otro barrido-----------------------------


class Otroscanner(QtCore.QObject):
    """Aca trato de armar un escaneo punto por punto."""
...
#        self.channelOrder = main.channelOrder
        ...
        AOchans = [0, 1, 2]
        # Following loop creates the voltage channels
        for n in range(len(AOchans)):  # mo olvido de z por ahora
            self.aotask.ao_channels.add_ao_voltage_chan(
                physical_channel='Dev1/ao%s' % AOchans[n],
                name_to_assign_to_channel='chan_%s' % self.channelOrder[n],
                min_val=minVolt[self.channelOrder[n]],
...
#            lines="Dev1/port0/line6", name_to_assign_to_lines='chan6',
#            line_grouping=nidaqmx.constants.LineGrouping.CHAN_PER_LINE)  # Ger
#        # cambie dotask para que simule ser un APD, cuando ande usar uno posta
    ...
    def runScan2(self):
        ...
        x0 = 0  # self.Positionner.x  # a confirmar si es asi. NO
        y0 = 0  # self.Positionner.y
#        stepSizeX = StageScan.parValues['stepSizeXY']
#        stepSizeY = StageScan.parValues['stepSizeXY']
#        print(float(Positionner.xlabel.text()), "x0??")
        sizeX = self.main.scanParValues['sizeX']
        sizeY = self.main.scanParValues['sizeY']
        self.sampleRate = 10**3
        self.seqSamps = int(np.round(self.sampleRate*tpix))

        barridoLTR = np.linspace(x0, sizeX, int(self.seqSamps/2))
        barridoRTL = np.linspace(sizeX, x0, int(self.seqSamps/2))
        barridox = np.append(barridoLTR, barridoRTL, axis=0)
#        barridox = np.linspace(x0, sizeX, self.seqSamps)
        barridoLTR = np.linspace(y0, sizeY, int(self.seqSamps/2))
        barridoRTL = np.linspace(sizeY, y0, int(self.seqSamps/2))
        barridoy = np.append(barridoLTR, barridoRTL, axis=0)
#        barridoy = np.linspace(y0, sizeY, self.seqSamps)
        cuentas = np.zeros((self.seqSamps, self.seqSamps))

        """cosas para simular si anda"""
        aidata = np.zeros((self.seqSamps, self.seqSamps, 2))
#        Napd = 25
...
#            if 0.5 < r:
#                signal[p] = True
#        APD = np.zeros(Napd, dtype='bool')
        """cosas para ver si anda ^^^^"""
        apdrate = 1000*1000
        Napd = int(tpix*apdrate)
        ...
        for j in range(self.seqSamps):
            tic = time.time()
            for i in range(self.seqSamps):
                self.aotask.write([barridox[i*((-1)**j)], barridoy[j], 0],
                                  auto_start=True)
...
#        auxi = np.linspace(0, self.seqSamps, self.seqSamps)
        data7 = aidata[:, 0, 0]
        data6 = aidata[:, 0, 1]
        for k in range(1, 2):  # self.seqSamps):
...
#        ax0.plot(self.stageScan.sigDict['z'], label="z")
        ax0.plot(data7, '.--y', label="data leida \" dev/ai7 \" ")
        ax0.plot(data6, '.--r', label="data leida \" dev/ai6 \" ")
        ax0.plot(aidata[:, 0, 0], '.--k', label="data leida \" dev/ai7 \" ")
        ax0.plot(aidata[:, 0, 0], '.--m', label="data leida \" dev/ai6 \" ")
#        ax0.plot(aidata[:, 1], '.-m', label="data leida \" dev/ai6 \" ")
#        ax0.plot(signal, '-c')
#        ax0.plot(didata, '.-c', label="data leida del digital ")
        ax0.legend(bbox_to_anchor=(1.15, 1.2))
