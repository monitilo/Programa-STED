...
# 'spectrum', 'cyclic', 'greyclip', 'grey'
    ...
    def paramChanged(self):
        ...
        self.pixelTime = float(self.pixelTimeEdit.text()) / 10**3  # ms to segs
...
# arma los barridos con los parametros dados
    def barridos(self):
        ...
        if self.step == 1:
            # arme barridos que hacen de a una linea a la vez y de a dos
...
#                   Barrido z (se queda en la posicion inicial)
        ...
        else:  # va y vuelve en un solo paso, ==> pasa de 2 en 2
            ...
            for i in range(self.numberofPixels):
                ...
                self.aotask.write(
                   [self.barridox[(i+1)*(-1)**self.dy],
                    self.barridoy[(i+1)] + Pasoy,
                    self.barridoz[i]], auto_start=True)
                tac = ptime.time()
                APD[i] = self.citask.read(number_of_samples_per_channel=self.Napd)
...
#                    self.cuentas[i] = aux + np.random.rand(1)[0]
        ...
        print("data posta", np.round(ptime.time() - tic, 5), "linetime = ", self.linetime)
...
# Se mueve a la posicion inicial de una manera agradable para el piezo
    ...
    def movetoStartStep(self):
        ...
        for i in range(len(volviendoy)):
            ...
            time.sleep(t / N)
        print(t, "vs", np.round(ptime.time() - tic, 5))
        self.dy = 0
...
#              samps_per_chan=10**8)
            ...
            self.citask.timing.cfg_samp_clk_timing(
              rate=apdrate, sample_mode=nidaqmx.constants.AcquisitionType.FINITE,
              source='20MHzTimebase',
              samps_per_chan = self.Napd * self.step * self.numberofPixels)
...
#                source=r'100kHzTimeBase',
                sample_mode=nidaqmx.constants.AcquisitionType.FINITE,
                samps_per_chan=int(self.step * self.numberofPixels))

    #        self.aotask.in_stream.input_buf_size = len(self.barridox)

    def rampas(self):
        if self.step == 1:
            # Para hacerlo de a lineas
            self.cuentas = np.zeros((self.numberofPixels))
...
#            self.stepSize = self.dy * self.pixelSize / convFactors['y']
    ...
    def vueltaX(self):
        ...
        startX = float(self.initialPosition[0])
        sizeX = self.scanRange + startX
        vueltax = np.linspace(sizeX, startX, self.numberofPixels)
        self.aotask.write(
                np.array([vueltax,
...
#        self.channelsOpen2()
        if self.step == 1:
            ...
            tac = ptime.time()
            APDtodo = self.citask.read(self.Napd * self.numberofPixels)

            toc = ptime.time()
#            self.cuentas[0] = APDtodo[self.Napd - 1]
            for j in range(self.numberofPixels-1):
    ...
    #        self.aotask.wait_until_done()
            print(np.round(ptime.time() - tic, 5))
            self.dy = 0
    
            aotaskramp.stop()
    #        self.aotaskramp.close()
