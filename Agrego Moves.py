...
#        self.edit_save.resize(self.edit_save.sizeHint())
    ...
    # Defino el tipo de Scan que quiero
        ...
        self.scanModes = ['ramp scan', 'step scan', 'otra frec ramp']  # no hay "otro"
...
#        print(self.linetime, "linetime")
        ...
        if self.scanMode.currentText() == "step scan":
            self.sampleRate = 42  # en el caso step no significan nada
            self.nSamplesrampa = 42
        ...
        if self.scanMode.currentText() == "otra frec ramp":
            self.sampleRate = self.apdrate 
            self.nSamplesrampa = self.sampleRate * self.linetime
            print("iguales frecuencias que apdrate\n",
                  self.nSamplesrampa, "Nsamples", self.sampleRate, "sampleRate")
...
# This is the function triggered by pressing the liveview button
    ...
    def liveviewStart(self):
        ...
        if self.scanMode.currentText() == "step scan":
            ...
            self.inStart = False
            self.tic = ptime.time()
            self.steptimer.start(100*self.pixelTime*10**3)  # imput in ms
...
#            tec = ptime.time()
            ...
            self.aotask.write(
             [self.allstepsx[(i+1)],  # *(-1)**self.dy
              self.allstepsy[(i+1)] + (self.dy * self.pixelSize / convFactors['y']),
              self.allstepsz[i]], auto_start=True)
...
#            self.cuentas[i] = aux + np.random.rand(1)[0]
            self.image[i, self.numberofPixels-1-self.dy] = APD[-1] - APD[0]  # *(-1)**self.dy
...
#                    self.guardarimagen()  # para guardar siempre (Alan idea)
    ...
    #       Barrido x
        ...
        self.allstepsx = (np.append(0, gox)) / convFactors['x']

    #       Barrido z (se queda en la posicion inicial)
        startZ = float(self.initialPosition[2])
        self.allstepsz = np.ones(len(self.allstepsx)) * startZ / convFactors['z']

    #       Barrido y
        startY = float(self.initialPosition[1])
        goy = np.ones(len(self.totalrampx))*startY
        self.allstepsy = np.append(0, goy) / convFactors['y']
